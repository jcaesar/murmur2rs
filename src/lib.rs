...
macro_rules! short_round {
    ($m:expr, $h:expr, $r:expr, $T:ty) => {{
        let r = $r;
        match r.is_empty() {
            false => ($h ^ r.iter().rev().fold(0, |r, &i| (i as $T) | (r << 8))).wrapping_mul($m),
            true => $h,
        }
    }};
}
...
pub fn murmur2(data: &[u8], seed: u32) -> u32 {
    ...
    let h = (&mut chunks).fold(h, |h, k| {
        round!(m, h, u32::from_le_bytes(k.try_into().unwrap()))
    });
    let h = short_round!(m, h, chunks.remainder(), u32);
...
pub fn murmur64a(data: &[u8], seed: u64) -> u64 {
    ...
    let h = short_round!(m, h, chunks.remainder(), u64);
...
pub fn murmur64b(data: &[u8], seed: u64) -> u64 {
    ...
    let (h1, h2) = (&mut chunks).fold((h1, h2), |(h1, h2), k| {
        (h2, round!(m, h1, u32::from_le_bytes(k.try_into().unwrap())))
    });
    let (h1, h2) = match len % 8 > 3 {
        true => (h2, h1),
        false => (h1, h2),
    };
    let h2 = short_round!(m, h2, chunks.remainder(), u32);
